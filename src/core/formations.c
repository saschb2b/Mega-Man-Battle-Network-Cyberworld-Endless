 ...
 * four tables, one per story stage, each holding for the real world and the
 * internet a list per map group, per map, of 16-byte BattleSettings ending
 * with 0xFF. An area takes every battle of its maps (RomLayout.net_area),
 * listed once, weighted by how many tables list it. */
...
static void add(int biome, uint32_t rec) {
	...
	if (count[biome] >= MAX_FORMATIONS) return;
